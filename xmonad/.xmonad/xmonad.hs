...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    , ((controlMask, xK_Print ), spawn "sleep 0.2; scrot -s '%Y%m%d-%H%M_$wx$h.png'  -e 'mv $f ~/screen_shots/'")
    -- Screenshot of the whole screen
    , ((0, xK_Print ), spawn "scrot '%Y%m%d-%H%M_$wx$h.png' -e 'mv $f ~/screen_shots/'")
    ...
    -- Toggle touchpad
    , ((modm, xK_F8 ), spawn "/sbin/trackpad-toggle.sh")
    ...
    --
    -- Toggle Suspend
    , ((modm .|. shiftMask, xK_s    ), spawn "/home/schmidmt/bin/toggle_suspend.sh -n")

    -- Summons a system monitor
    , ((modm, xK_F2), spawn "xterm -e htop")

    -- Jump to the fullscreen layout
    , ((modm, xK_f), sendMessage $ JumpToLayout "Full")

    -- Jump to sprial layout
    , ((modm, xK_s), sendMessage $ JumpToLayout "Spiral")
...
--    , className =? "Gimp"           --> doFloat -- Make gimp's windows float
    , className =? "Zenity"           --> doFloat -- Make zenity's windows float
